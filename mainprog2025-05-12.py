...
def up_photo(photo_list):
	...
	log("Up pressed - photo_increment="+photo_increment, 1)
...
def down_photo(photo_list):
	...
	log("Down pressed - photo_increment="+photo_increment, 1)
	menu(list)
	return photo_increment
...
def menu(list):
	...
	for i in list:
		if i==list[h]:
			i="> "+i
		draw.text((20,y),i+"\n",font=font,fill=0)
		y=y+25 # Increment y position for next item
...
#	draw.text((5, 280), filename, font=font, fill=1)
	success=epd.display(epd.getbuffer(image))
	if success:
		log("Displayed file: "+filename,1)
	else: 
		log("Could not display file: "+filename,0)
...
def autoscroll_time_calc():
	...
	log("Timestamp +autoscroll_duration: "+str(future))
	list_made=True
	return
...
# draw=ImageDraw.Draw(image) # !!!!!!! NEED ???????
