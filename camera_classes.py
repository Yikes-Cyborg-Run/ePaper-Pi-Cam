...
class Display():
	...
	def check_delete(self, data, photo_list):
		num=data[3]
		if data[2]==False:
...
class Menu():
	...
	def build(self, h, sel, photo_list):
		...
		for item in use_menu:
			...
			if item=='Autoscroll' or item=='Manual Scroll' or item=='Purge' or item=='Delete' : # ??????? I dont think Delete is needed - it's called from Manual Scroll
...
#				Action().display_photo(photo_list[num])
	...
	def archive_confirmed(self):
		...
		data=Action().purge_photo_dir(len(self.image_dir) , False) # False - don't need to show purge message
		return data

	# Function to delete ALL photos on file
	# I used the word "PURGE" to make it stand out against the word delete (for single photo).
	def purge_photo_dir(self, num_photos, show_message):
