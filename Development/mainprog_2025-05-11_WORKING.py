...
def menu(menu_title, items, up, down, func):
	global config_font, h, selection
	old=items[h]
	limit=len(items)-1
	if up==False: h+=1
	elif down==False: h-=1
		...
		selection=items[h]
		h=0
		log("Selected: "+str(selection)+" - h: "+str(h),1)
		return
	if h==0:h=1 # remove "Menu" option
    # Check menu increment
	if h>limit: h=0
	elif h<0: h=limit
	new=items[h]
	# Highlight menu item
#	log("New: "+new+" - Old: "+old,1)
	if new!=old:
		y=15 # Will increment to place menu items vertically
		...
		draw.text((15,10),menu_title, font=font, fill=0)
...
def config_menu(menu_title, config, items, up, down, func):
	log("Config menu loaded",1)
	global config_font, h, selection
	old=items[h]
	limit=len(items)-1
	if up==False: h+=1
		...
		new_value=items[h]
		if menu_title=="Autoscroll Duration":
			config["autoscroll_duration"]=new_value
		elif menu_title=="Display Rotaion":
			config["display_rotation"]=new_value
		elif menu_title=="Font Selection":
			config["font"]=new_value
		elif menu_title=="Timestamp Photo":
			config["timestamp_photo"]=new_value
		elif menu_title=="Brightness":
			config["brightness"]=new_value
		elif menu_title=="White Balance":
			config["whitebalance"]=new_value
		elif menu_title=="Shut Down":
			log("Shut Down",1)
		save_config(config)
...
autoscroll_duration=config["autoscroll_duration"]
...
#main_menu_array=["Menu", "Camera", "Camera Options", "Autoscroll", "Manual Scroll"]
main_menu_array=["Menu", "Camera", "Autoscroll", "Manual Scroll", "Delete Photos"]
options_menu_array=["Camera Options", "Font Selection", "Display Rotation", "Autoscroll Duration", "White Balance", "Shut Down", "Delete Photos"]
white_balance_array=["auto", "tungsten", "fluorescent", "indoor", "daylight", "cloudy"]
autoscroll_array=[10, 30, 60, 120, 300, 600]
...
try:
	while True:
		...
		# If not on menu and menu btn is pressed, change selection to show menu
		...
		if selection=="Menu":
			...
			menu("MENU - Photos: "+str(len(photo_array)), main_menu_array, up_state, down_state, photo_state)

		# Options Menu
		elif selection=="Camera Options":
#			print("Camera Options")
			menu("Camera Options", options_menu_array, up_state, down_state, photo_state)
		...
		# Autoscroll
			...
			if len(photo_array)>0:
				...
				time.sleep(5)
				list_increment+=1
				if list_increment>=len(photo_array)-1: # removed int() around photo array
					list_increment=0
			else:
				if list_made==False:
					list_made=True
			...
			log("Autoscroll Duration Menu")
			config_menu("Autoscroll Duration", config, autoscroll_array, up_state, down_state, photo_state)

		elif selection=="Timestamp Photo":
			menu("Timestamp Photo", timestamp_array, up_state, down_state, photo_state)
			####### SAVE CONFIG !!!!!!!
		...
		# Ask to confirm purging all photos
			...
			if check_delete==False:
				...
				image=Image.new("1", (epd.height, epd.width), 255) 	# Create a new image with a white background
				draw=ImageDraw.Draw(image)
				font=ImageFont.truetype(config_font, 20)
				draw.text((15,5),"Are you SURE you want to \ndelete all "+str(len(photo_array))+" photos?",font=font,fill=0)
				font=ImageFont.truetype(config_font, 16)
