...
#	draw=draw.transpose(Image.ROTATE_180)
	...
	#font=ImageFont.truetype("/home/pi/ePaper-Pi-Cam/"+config_font, 18)
	...
	for item in menu_array:
		if item==highlight:
			show="- "+item
			if item=="Camera":
				show="- Camera - push btn to take photo..."
			elif item=="List":
				show="- List - "+str(num_photos)+" photos..."
		else:
			show=item
		if item!="Menu":
			draw.text((20,y),show+"\n",font=font,fill=0)
